...
template <typename T> class LargeSparseMatrix {
	public:
		...
		virtual sparse_row<T>& get(uint& x) = 0; // pointer to the row with specific index
...
//				assert(fh.float_size == sizeof(T));
				if((fh.id != FMATRIX_EXPECTED_FILE_ID) || (fh.float_size != sizeof(T))){
					 throw 69;
				}
...
//		~LargeSparseMatrixHD() { in.close(); }
		...
		virtual sparse_row<T>& get(uint& x){
			return data(x);
		}
...
template <typename T> class LargeSparseMatrixMemory : public LargeSparseMatrix<T> {
	...
	public:
		...
		virtual sparse_row<T>& get(uint& x){ return data(x); };
